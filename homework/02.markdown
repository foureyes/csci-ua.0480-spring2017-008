...
Do not use a regular `for`, `for ... in`, `for ... of`, or `forEach` loop. __There will a small (-2) penalty every time one is used__.
...
### (-2 per for, forEach, for of, or for in loop used)
...
* `num1`, `num2`, up through ... `numN` - the values to be summed
...
4. it'll read a file using `fs.readFile` (make sure the `fs` module is `require`'d in your project)
    ...
    * the callback supplied to `readFile` should:
        ...
        * call the callback passed into the new function with the err object (regardless of wheteher or not it's null/undefined) and the parsedData (again, regardless of whether or not it has a valud or is `undefined`)
...
* Example string:
    <pre><code data-trim contenteditable> * Player with highest 3 point percentage that took at least 5 shots: Trevor Ariza at %49.50 (4/8) 
</code></pre>
...
* Player with highest 3 point percentage that took at least 5 shots: Trevor Ariza at %49.50 (4/8)
...
* The URL for the JSON data associated with this game is at:
    * [http://foureyes.github.io/csci-ua.0480-spring2017-008/homework/02/0021600680_gamedetail.json](http://foureyes.github.io/csci-ua.0480-fall2016-001/homework/02/0021600680_gamedetail.json')
