...
# <span class="warning">Draft (Mostly Finished)</span>
...
* create and apply the appropriate CSS classes to get rid of the _form_ (do this with styles, there's no need to remove the element) to make room for displaying cards
...
* Note that there was a text field in the original form - this can be used to set the cards at the top of the deck 
    ...
    * The input should be a comma separated list of characters and/or numbers (for example `K,2,3,7`)
...
* If there were card faces that were entered in the form above...
    * add those cards to the top of the deck
...
* Deal the cards - alternate between the computer and player
    ...
    * the player gets the second card
...
1. pressing Hit should deal the next card from the deck
    * if Hitting makes a user's total go over 21, then the user's turn ends immediately 
...
2. The player with the score closer to 21 wins
...
* when a hand ends, add a restart button along the bottom row of buttons
