...
There's one last method that we'll implement on `Request`:
...
1. Determine the absolute path to the file that's passed in as its argument by using `__dirname` to find the directory of the module... and go up one so that public can be accessed.
...
2. Use the [specifications for the `App` object below](#app)
