...
Set up an express project that serves static files (with `express.static`), is able to handle forms that POST (with `body-parser`), uses templating (`hbs`) and has session handling (`express-session`). Put all of this in a folder called `ait-news`, and in a file called `app.js`. __Use dynamically generated paths for your views and static files by using a special built in variable, `__dirname`:__ &rarr;
...
```
And finally, __make sure to listen on port 3000__ (please make sure to keep this port number; graders will look for your app on localhost:3000)


Within the ait-news directory, we'll be using a couple of tools to help with database access and creating _url slugs_:
